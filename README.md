...
#### Gradient Boosting
The final model created was a Gradient Boosting Model. Gradient Boosting is a ensemble learning method which combines several weak learners into strong learners. The model is trained sequentially, and each new model is trained to minimize the loss function of the preceding model, in our case mean squared error. The predictions from these models are then combined together into one model. Gradient boosting is theoretically more accurate compared to other models, but it often must be optimized in order to outperform them. This improved performance comes at the cost of a greater risk of overfitting, but we circumnavigated this issue by using a low learning rate. The Gradient Boosting Model with default hyperparameters performed moderately well with a RMSE of 55267.66 and a Coefficient of Determination Regression Score of 0.7689. While not as accurate as the unoptimized and optimized Random Forest Model, it still outperformed both iterations of the Linear Regression Model by a comfortable margin. When using Cross Validation to optimize the hyperparameters, performance increased significantly to a RMSE of 46889.00 and a Coefficient of Determination Regression Score of 0.8337.
#### Comparing Models
Unsurprisingly, the Linear Regression Model had the worst accuracy of all of our models. Due to the lack of multicollinearity of our model, adding the ridge regression optimization did little to improve the performance. While there are other ways to improve performance of a Linear Regression Model, the improvements would be insufficient to improve the performance to the same level as Random Forest or Gradient Boosting. When using default hyperparamaters the Random Forest Model outperformed the Gradient Boosting Model, but once both were optimized the Gradient Boosting performance was marginally superior. After working with and optimizing all of the models, we determined that an acceptable margin for error would be a Coefficient of Determination Regression score above 0.8, which was met by both variations of the Random Forest Method and the optimized Gradient Boosting Method. 
